...
//            client.setScreen(customScreen);
        ...
        }).dimensions(RenderingPositionY.getX() + RenderingPositionX.getWidth(), 140 - ButtonDistance - ButtonHeight, 12, ButtonHeight).tooltip(Text.literal("是否开启最大强度显示")).build();
...
//        MaxStrength = ButtonWidget()
        ...
        saveFile = ButtonWidget.builder(Text.literal("保存配置到文件"), button -> {
                ...
                .dimensions((int) ((double) width / 2 - (width * 0.4) - 5), 20, (int) (width * 0.4), ButtonHeight).tooltip(Text.literal("所有更改是临时更改\n点击此按钮保存到文件")).build();
        ...
        webSocketConfig = ButtonWidget.builder(Text.literal("连接设置"), button -> {
                ...
                .dimensions(width / 2 + 5, 20, (int) (width * 0.4), ButtonHeight).tooltip(Text.literal("点击修改连接设置\n非必要无需修改")).build();
        ...
        }).dimensions((int) ((double) width / 2 - (width * 0.4) - 5), 20 + ButtonHeight + ButtonDistance, (int) (width * 0.4), ButtonHeight).tooltip(Text.literal("点击修改强度设置")).build();
        ...
        }).dimensions(width / 2 + 5, 20 + ButtonHeight + ButtonDistance, (int) (width * 0.4), ButtonHeight).tooltip((Text.literal(":P"))).build();
